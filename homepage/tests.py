...
class HomepageIndexViewTests(TestCase):
    def setUp(self):
        ...
        self.city_spb = City.objects.create(name='Санкт-Петербург', region=self.region_spb)
        self.city_msk = City.objects.create(name='Москва', region=self.region_msk)

        self.district_spb = District.objects.create(name='Приморский', city=self.city_spb)
        self.district_msk = District.objects.create(name='Тверской', city=self.city_msk)

        self.developer = Developer.objects.create(name='Developer')
        self.real_estate_type = RealEstateType.objects.create(name='Квартира')
        self.real_estate_class = RealEstateClass.objects.create(name='Комфорт', weight=Decimal('1.00'))
    ...
    def test_index_filters_complexes_by_selected_city(self):
        response = self.client.get(reverse('homepage:index'), {'city': self.city_msk.id})
