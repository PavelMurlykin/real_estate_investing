...
export function SavedMortgageCalculationListPage() {
  useDocumentTitle('История расчётов ипотеки')
    ...
    <div className="page-stack">
      <header className="page-header">
        <div>
          <span className="eyebrow">Личный раздел</span>
          <h1>История расчётов ипотеки</h1>
          <p>Сохранённые сценарии доступны владельцу аккаунта и готовы для повторного анализа.</p>
        </div>
      ...
      {results.length === 0 ? (
        ...
        <>
          ...
          <div className="property-mobile-list">
            {results.map((calculation) => (
              <CalculationMobileCard calculation={calculation} key={calculation.id} />
            ))}
          </div>
        </>
      )}
