...
сессию. Восстановление пароля по email перенесено полностью: запрос не
раскрывает существование аккаунта, отправка ограничена по частоте, одноразовый
токен после проверки заменяется session-bound разрешением и удаляется из URL,
а завершение сбрасывает текущую сессию. Старые страницы регистрации, профиля,
смены и восстановления пароля доступны как резервные до завершения
приёмочного тестирования.
Общая React-оболочка сохраняет верхнюю и левую навигацию во время загрузки
страниц и при ошибке загрузки модуля. Текущий раздел выделяется в боковой
панели, включая вложенные карточки и вкладки справочников. На экранах до
960 px меню открывается как модальный диалог с управлением фокусом, Escape,
закрытием по клику снаружи и блокировкой прокрутки фона. Переходы между
страницами переводят фокус в содержимое; изменение фильтров в URL не сбрасывает
фокус и прокрутку. Ошибка страницы предлагает обновление и переход на главную.
React-маршруты доступны по
адресам /app/, /app/login, /app/register, /app/profile,
/app/password/change, /app/password/reset,
/app/password/reset/:userIdentifier/:token, /app/password/reset/confirm,
/app/properties, /app/properties/new,
...
Браузерная приёмка выполняется на запущенной production-сборке в отдельных
анонимных сессиях Chromium. Тесты блокируют изменяющие API-запросы и проверяют
прямую загрузку публичных маршрутов на 360 и 1280 px, отсутствие горизонтального
переполнения, ограничения приватных страниц, возврат на исходный URL после
входа, поиск и сортировку через URL, историю браузера, повтор API-запроса и 404.
Отдельные сценарии проверяют активные разделы, модальное меню на 360 и 780 px,
клавиатуру, смену ширины экрана, широкие таблицы, задержку и ошибку загрузки
страницы.
...
$env:BROWSER_TEST_BASE_URL='http://127.0.0.1:18080'
npm run test:browser
```

Для системного Chromium можно задать `CHROMIUM_EXECUTABLE_PATH`. Необязательная
переменная `BROWSER_TEST_SCREENSHOTS` задаёт каталог снимков экрана. При
проверке через Docker Desktop используйте адрес `http://localhost:18080`
и `BROWSER_TEST_CONNECT_HOST=host.docker.internal`: Chromium разрешит
localhost в адрес хоста Docker, сохраняя корректный HTTP Host.
Браузерные проверки дополняют компонентные тесты. Для контролируемого
переключения старых HTML-страниц подготовлен выключенный по умолчанию флаг
`REACT_FRONTEND_LEGACY_REDIRECTS_ENABLED`. При значении `True` безопасные
GET/HEAD-запросы к перенесённым Django-страницам получают временный redirect на
соответствующий `/app/...` с сохранением query-параметров. POST-запросы,
версионированный API, AJAX-endpoints и выгрузки продолжают обслуживаться
Django. Параметры `model` и `edit` объединённых каталогов преобразуются в
конкретную React-вкладку и маршрут редактирования. Шаблоны и Python-обработчики
не удаляются, поэтому для отката достаточно вернуть флаг в `False` и
перезапустить web-процесс.

Авторизованные сценарии с записью запускаются только на отдельном временном
стенде PostgreSQL. Стенд доступен исключительно на `127.0.0.1:18081`, не
подключает `.env` и рабочие тома, а тестовые пароли и Django secret генерируются
заново при каждом запуске. На этом стенде cutover-флаг включён принудительно:
Chromium открывает старые публичные, приватные и объединённые каталожные URL,
проверяет их переход на правильные React-маршруты и отсутствие redirect для API
и небезопасных запросов. Затем Chromium проходит вход через React, CRUD групп
компаний и клиентов на 360 и 1280 px, а также сквозной цикл застройщика, ЖК с
корпусом и объекта недвижимости, банковские формы с вложенными условиями,
ипотечные программы с региональными лимитами и алиасами, программы застройщиков
и каскадные справочники локаций. Сценарии проверяют сохранение связей после
редактирования, все справочники объектов, идемпотентный XLSX-импорт и полный
цикл траншевой ипотеки с Excel/Word. Также проверяется запрет удаления
используемых ЖК и локаций.
Приёмка проверяет ролевой запрет и изоляцию
персональных данных, CSRF, выход, сохранение ипотечного расчёта и обе выгрузки.

```powershell
.\scripts\run-react-browser-acceptance.ps1
```

Скрипт намеренно оставляет изолированный стенд работающим для ручной проверки.
После проверки удалить только его контейнеры и временные тома можно командой:

```powershell
docker compose -f compose.browser-tests.yaml down --volumes
```

Тесты с записью требуют одновременно специальный маркер, manifest фикстур,
сгенерированный пароль и loopback-адрес с портом 18081. Без этих условий они
пропускаются и не могут изменить обычный локальный или production-стенд.
