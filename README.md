...
детальную карточку объекта с галереей изображений, приватный список, карточку,
создание и редактирование клиентов с оценкой финансового потенциала, а также
...
расчёта. Траншевый расчёт также сохраняется из React, а его история и
детальная карточка пока открываются в сохранённой Django-версии.
Сохранённый рыночный сценарий можно повторно открыть как образец, выгрузить
в Excel или Word и удалить после явного подтверждения. React-маршруты доступны по
адресам /app/, /app/properties, /app/properties/:id, /app/customers,
/app/customers/new, /app/customers/:id, /app/customers/:id/edit,
/app/mortgage, /app/mortgage/trench, /app/mortgage/calculations и
/app/mortgage/calculations/:id.
API имеет версию /api/v1/ и использует текущие Django session authentication,
CSRF-защиту, правила доступа и PostgreSQL. Удаление клиентов, связанные
расчёты и Word-отчёты пока явно открываются в сохранённой Django-версии.
Старые формы создания и редактирования клиентов также не удалены. Старая
форма /mortgage/ сохранена для проверки функционального паритета обоих видов
ипотеки до завершения миграции.
