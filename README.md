...
детальную карточку объекта с галереей изображений, а также рыночный ипотечный
калькулятор с банковскими программами и графиком платежей, сохранение расчёта
для выбранного объекта, приватную историю и страницу детального расчёта.
...
адресам /app/, /app/properties, /app/properties/:id, /app/mortgage,
/app/mortgage/calculations и /app/mortgage/calculations/:id.
API имеет версию /api/v1/ и использует текущие Django session authentication,
CSRF-защиту, правила доступа и PostgreSQL. Старая форма /mortgage/ сохранена
для траншевых расчётов и проверки функционального паритета до завершения
миграции.
