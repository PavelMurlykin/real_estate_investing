...
def mortgage_calculator(request):
    ...
    if request.method == 'POST' and 'calculate' in request.POST:
        ...
        if form.is_valid():
            ...
            context['payment_schedule'] = calculator.get_payment_schedule()
            context['form'] = form
        ...
        if form.is_valid():
            ...
            input_data = [
                ...
                ['Дата первоначального взноса',
                    data['INITIAL_PAYMENT_DATE'].strftime('%d.%m.%Y')],
                ['Срок ипотеки, годы', data['MORTGAGE_TERM']],
                ['Годовая ставка, %', data['ANNUAL_RATE']],
                ['Наличие льготного периода',
                    'Да' if data['HAS_GRACE_PERIOD'] == 'да' else 'Нет'],
            ]
            ...
            result_data = [
                ...
                ['Дата последнего платежа по льготному периоду',
                    result['grace_period_end_date'].strftime('%d.%m.%Y')],
                ['Сумма ежемесячного платежа во время льготного периода, руб.',
                    result['grace_monthly_payment']],
                ['Сумма кредита после окончания льготного периода, руб.',
                    result['loan_after_grace']],
                ...
                ['Сумма ежемесячного платежа за основной период, руб.',
            ...
            for row, payment in enumerate(payment_schedule, start=schedule_start+2):
                ...
                ws.cell(row=row, column=3, value=payment['payment_amount'])
                ws.cell(row=row, column=4, value=payment['interest_amount'])
                ws.cell(row=row, column=5, value=payment['principal_amount'])
                ws.cell(row=row, column=6, value=payment['remaining_debt'])
