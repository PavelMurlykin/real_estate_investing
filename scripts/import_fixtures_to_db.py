...
def _topological_models(project_models):
    ...
    ready = sorted((model for model, degree in in_degree.items() if degree == 0), key=_model_sort_key)
    ordered = []
    ...
    if len(ordered) != len(project_models):
        ...
        print(
            "[WARN] Circular model dependencies detected. Remaining models will be loaded "
...
def import_fixtures() -> int:
    ...
    print(f"Import complete. Loaded {loaded_count} fixture files. Missing files: {missing_count}.")
    return 0
