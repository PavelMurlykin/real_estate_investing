...
class Customer(BaseModel):
    ...
    birth_year = models.PositiveSmallIntegerField(null=True, blank=True, verbose_name='Год рождения')
    ...
    desired_floor = models.CharField(max_length=100, blank=True, verbose_name='Этаж')
    cardinal_directions = models.CharField(max_length=255, blank=True, verbose_name='Стороны света')
    ...
    def clean(self):
        ...
        if not self.first_name or not self.first_name.strip():
            raise ValidationError({'first_name': 'Поле "Имя" обязательно для заполнения.'})
        ...
        if self.birth_year and self.birth_year > today.year:
            raise ValidationError({'birth_year': 'Год рождения не может быть в будущем.'})
        ...
        if self.area_min is not None and self.area_max is not None and self.area_min > self.area_max:
            raise ValidationError({'area_max': 'Максимальная площадь должна быть не меньше минимальной.'})
        ...
        if (
            ...
            raise ValidationError({'desired_district': 'Район должен относиться к выбранному городу.'})
    ...
    def get_actual_cbr_key_rate(cls):
        rate = KeyRate.objects.order_by('-meeting_date').values_list('key_rate', flat=True).first()
    ...
    def calculate_max_property_cost(self, annual_rate=None, max_term_years=None):
        ...
        term_years = self.get_max_mortgage_term_years() if max_term_years is None else int(max_term_years)
        if term_years <= 0:
        ...
        if monthly_payment > 0:
            if monthly_rate == 0:
                ...
                credit_amount = monthly_payment * (Decimal('1') - discount_factor) / monthly_rate

        total_property_cost = down_payment + credit_amount
        if total_property_cost <= 0:
            return None
        return total_property_cost.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
